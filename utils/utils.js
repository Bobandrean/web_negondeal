...
//showPopups({
// msg: "Sukses tambah unit"
//})

const showSuccessPopup = ({ msg }) => {
  const _msg =
    msg?.response?.data?.message || msg?.message || msg || "Berhasil";
...
const showFailedPopup = ({ msg }) => {
  ...
  return Swal.fire({
    title: msg?.response?.data?.status || "ERROR",
    text: _msg,
    icon: "error",
    showConfirmButton: false,
    timer: 1500,
...
const showPopups = ({ msg }) => {
  const _msg =
    msg?.response?.data?.message || msg?.message || msg || "Terdapat Kesalahan";

  return Swal.fire({
    title: msg?.response?.data?.status.toUpperCase() || _msg,
    // text: _msg,
    icon: "info",
    showConfirmButton: false,
    // timer: 1500,
